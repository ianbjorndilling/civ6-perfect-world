...
function GeneratePlotTypes()
	...
	for y = 0, gridHeight - 1,1 do
		for x = 0,gridWidth - 1,1 do
			...
			local plot = Map.GetPlot(x, y);
			if elevationMap:IsBelowSeaLevel(x,y) then
				plot:SetPlotType(g_PLOT_TYPE_OCEAN, false, false)
			elseif diffMap.data[i] < hillsThreshold then
				plot:SetPlotType(g_PLOT_TYPE_LAND,false,false)
			elseif diffMap.data[i] < mountainsThreshold then
				plot:SetPlotType(g_PLOT_TYPE_HILLS,false,false)
			else
				plot:SetPlotType(g_PLOT_TYPE_MOUNTAIN,false,false)
			end
		end
	end
		...
		for x = 0,gridWidth - 1,1 do
			...
			if not elevationMap:IsBelowSeaLevel(x,y) then
				if plot:GetTerrainType() == terrainSnow then
					...
					if lowerFound and plot:GetPlotType() == g_PLOT_TYPE_LAND then
						plot:SetPlotType(g_PLOT_TYPE_HILLS,false,false)
					end
				elseif plot:GetTerrainType() == terrainTundra then
					...
					if higherFound and plot:GetPlotType() == g_PLOT_TYPE_HILLS then
						plot:SetPlotType(g_PLOT_TYPE_LAND,false,false)
					end
				end
			end
...
function GenerateMap()
    ...
    terrainTypes = GenerateTerrainTypes()
